...
export const getServices = cache(async () => {
  return await fetchAPI(
    "services?_fields=id,slug,yoast_head_json,date,modified,title,content&per_page=100",
    {
      next: { revalidate: 60 },
    }
  );
...
export const getServiceBySlug = cache(async (slug) => {
  return await fetchAPI(
    `services?slug=${slug}&_fields=id,slug,yoast_head_json,date,modified,title,content`
  );
});
